...
class LTCForecaster(pl.LightningModule):
    def __init__(self, input_size=6, ncp_units=32, lr=0.01, use_wb_equations=True, mixed_memory=True):
        ...
        parameter:
        input_size: input_size: the number of original input features
        ...
        # Compute correct input dimensions for wiring.build()
        ...
        for t in range(seq_len):
            ...
            if self.use_wb_equations:
                ...
                for i in range(augmented_inputs.size(1)):
                    # For each input feature, calculate its correlation with the neuron state
...
def visualize_neuron_dynamics(activations, wiring, output_dir, step=1):
    ...
    for i in range(min(wiring.units, 10)):
    ...
    # Visualizing neuron activation heatmaps
...
def visualize_attention_weights(attention_weights, feature_names, output_dir):
    ...
    if len(feature_names) != actual_feature_count:
        ...
        # Adjust feature name list to correct length
        if len(feature_names) > actual_feature_count:
...
def visualize_predictions_with_confidence(test_true, test_pred, scaler, time_range, output_dir):
    ...
    # 计Calculate evaluation indicators
...
def visualize_water_balance_equations(model, output_dir):
    """Visualize water balance equation components and parameters"""
    ...
    # Define component positions and sizes - Added karst aquifer
    components = {
        ...
        'gravity': (0.5, 0.35, 0.3, 0.1),  
        'karst': (0.5, 0.20, 0.3, 0.1),  
    ...
    # Draw Arrow Connections - Add a karst aquifer connection
    arrows = [
    ...
    for start, end, label in arrows:
        ...
        # Draw a curved arrow if the start and end x coordinates are different
        if start_x != end_x:
        ...
        # Add offset to avoid overlapping with arrows
        if start_x != end_x:
    ...
    # Add Key Parameters text box - updated to include new karst parameters
    param_text = "\n".join([
    ...
    # Add geological layering diagram
...
def create_visualization_dashboard(model, dataloader, test_pred, test_true, scaler, output_dir):
    ...
    print("Start creating a visualization dashboard盘...")
    ...
    print("2/8 Visualize prediction results and confidence intervals间...")
    ...
    print("3/8 Compute and visualize feature importance...")
    ...
    for x, y, times in dataloader:
        ...
        if model.use_wb_equations:
            ...
            feature_names = ['pre', 'tep', 'day_length', 'ext', 'src', 'sp']  # Does not include water balance output
...
def create_html_index(output_dir):
    """Creating an HTML Results Index Page"""
    # Get all PNG files
...
def main():
    ...
    # Modify the data loader to add processing for time intervals
    ...
    print(f"Please open {os.path.join(vis_output_dir, 'index.html')} to view the complete visualization report")
